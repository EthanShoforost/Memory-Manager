...
def fix_file_metadata(file_path, date_string):
    """Fix metadata for downloaded file"""
    try:
        # Parse datetime from the date string (already in local time from convert_utc_to_local)
        ...
        # Update file modification time for all files (images and videos)
            ...
            try:
                ...
                if is_zip:
                    ...
                    if media:
                        ...
                        # Fix metadata
                        fix_file_metadata(out, safe_date)
                        with lock: self.stats['processed'] += 1
                    shutil.rmtree(temp, ignore_errors=True)
                else:
                    out = os.path.join(self.output_dir, f"{safe_date}.{'mp4' if media_type.lower()=='video' else 'jpg'}")
